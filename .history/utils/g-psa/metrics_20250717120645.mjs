...
export const indentationsMetric = ({ entries }) => {
  let totalIndent = 0;
  let totalLines = 0;
  let maxIndent = 0;

  for (const entry of entries) {
    if (entry.type !== 'file' || !entry.indentations) continue;

    for (const indent of entry.indentations) {
      totalIndent += indent;
      totalLines++;
  ...
  const averageIndent = totalLines ? +(totalIndent / totalLines).toFixed(2) : 0;
  ...
  return {
    indentations: {
      maxIndentation: {
        number: maxIndent,
        toString() {
          return `max indentation: ${this.number} spaces`;
        }
      },
      averageIndentation: {
        number: averageIndent,
        toString() {
          return `average indentation: ${this.number} spaces`;
        }
      }
    }
  };
